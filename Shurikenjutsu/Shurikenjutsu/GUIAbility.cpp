...
bool GUIAbility::Initialize(DirectX::XMFLOAT3 p_position, float p_width, float p_height, ID3D11ShaderResourceView* p_texture, std::string p_hotKey)
{
	...
	m_stacks.Initialize("", L"RagingRedLotus BB", 25.0f, p_position.x, p_position.y + 36.0f, 0xff071e31);
	m_hotKeyText.Initialize(p_hotKey, L"RagingRedLotus BB", 20.0f, p_position.x, p_position.y - 35.0f, 0xff071e31);
	m_cooldownText.Initialize("", L"RagingRedLotus BB", 36.0f, p_position.x, p_position.y, 0xff0000ff);
	m_lastnrofStacks = -1;
	return true;
}
