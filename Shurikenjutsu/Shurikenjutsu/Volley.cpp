...
void Volley::Update()
{
	...
	for (unsigned int i = 0; i < m_projectiles.size(); i++)
	{
		m_projectiles[i]->Update(m_timer);
	}
...
void Volley::AddRandomProjectiles(int p_amount, DirectX::XMFLOAT3 p_startPosition, DirectX::XMFLOAT3 p_endPosition)
{
	...
	{
		DirectX::XMFLOAT3 endPosition = DirectX::XMFLOAT3(p_endPosition.x + cosf(angle*i)*1.0f, p_endPosition.y, p_endPosition.z + sinf(angle*i)*1.0f);
