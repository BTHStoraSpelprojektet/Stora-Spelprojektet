...
std::vector<DirectX::XMFLOAT4X4> AnimationControl::UpdateAnimation()
{
	...
	if (m_frameArms >= (m_animationStacks[2].m_endFrame - 1))
		m_frameArms = 0.0f;
	...
	int* index = new int(0);
	CombineMatrices(index, m_animationStacks[2].m_root[(int)m_frameArms], m_animationStacks[0].m_root[(int)m_frameLegs], startQuaternion, startTranslation);
	delete[] index;
...
{
	DirectX::XMVECTOR quaternion = DirectX::XMVectorSet(p_jointArms->m_quaternion[0], p_jointArms->m_quaternion[1], p_jointArms->m_quaternion[2], p_jointArms->m_quaternion[3]);
