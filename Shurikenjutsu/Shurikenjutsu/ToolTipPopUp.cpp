...
void ToolTipPopUp::Initialize(float p_xPos, float p_yPos, std::string p_text , float p_size, float p_backGroundWidth, float p_backGroundHeight ,float p_bgOffset)
{
	...
	m_toolTipBG->Initialize(p_xPos, p_yPos + 50.0f + p_bgOffset, p_backGroundWidth, p_backGroundHeight, TextureLibrary::GetInstance()->GetTexture((std::string)"../Shurikenjutsu/2DTextures/GUI/abilitybarLobby.png"));
...
void ToolTipPopUp::Update(float p_xPos, float p_yPos, float p_size)
{
